...
export async function deleteStudents(ids: number[]) {
  ...
  return await db.transaction(async (tx) => {
    // 1. 删除相关面试
    await tx.delete(schema.interviews).where(inArray(schema.interviews.studentId, ids));
    // 2. 删除相关评论
    await tx.delete(schema.comments).where(inArray(schema.comments.studentId, ids));
    // 3. 删除学生本身
    return await tx.delete(schema.students).where(inArray(schema.students.id, ids));
  });
}
...
export async function getAiSettings() {
  ...
  try {
    ...
    const s = settings[0];
    const dbBaseUrl = s.llmBaseUrl || '';
    const dbApiKey = s.llmApiKey || '';
    const dbModel = s.llmModel || '';
    ...
    return {
      ...
      llm: {
        // 如果数据库里的 URL 是默认的 OpenAI 且没有设置 API Key，则自动回退到 .env 中的系统配置 (DashScope)
        baseUrl: (dbBaseUrl === 'https://api.openai.com/v1' && !dbApiKey) ? dashscopeBaseUrl : (dbBaseUrl || dashscopeBaseUrl),
        apiKey: dbApiKey || dashscopeApiKey,
        model: dbModel || dashscopeModel
      }
    };
  } catch (error) {
    ...
    return {
      vision: {
        ...
        model: process.env.VISION_MODEL || 'vision-vk-v2',
